...
<section class="py-24 {{ $bgClasses }} relative overflow-hidden">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative text-center">
        @if($badge)
        <span class="text-black font-black text-xs uppercase tracking-[0.3em] mb-6 block">{{ $badge }}</span>
        @endif
        <h2 class="text-4xl md:text-6xl font-black font-display text-black leading-tight mb-10">{!! $title !!}</h2>
        <div class="flex flex-col sm:flex-row items-center justify-center gap-6">
            <a href="{{ $buttonUrl }}" target="_blank" class="group relative px-12 py-6 bg-black text-white font-black text-sm uppercase tracking-[0.2em] rounded-full hover:scale-105 transition-all shadow-2xl">
