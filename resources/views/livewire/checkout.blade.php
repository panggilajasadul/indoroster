            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                @guest
                    ...
                    <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 sm:p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-blue-900 shadow-sm">
                        ...
                        <a href="{{ route('login') }}" class="font-display shrink-0 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 px-4 py-2.5 rounded-xl transition-colors">
                            Masuk / Daftar
                        </a>
                    </div>
                @endguest
                        ...
                        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 sm:p-8">
                            ...
                            <div class="grid grid-cols-1 gap-4">
                                @foreach($savedAddresses as $addr)
                                    <label class="relative flex p-4 border rounded-xl cursor-pointer focus:outline-none transition-all duration-200 {{ $selectedAddressId == $addr->id ? 'border-terra-500 bg-terra-50/30' : 'border-gray-200 hover:bg-slate-50' }}">
                                        ...
                                        <span class="flex flex-col text-left">
                                            <span class="flex items-center gap-2">
                                                <span class="font-display font-bold text-sm text-slate-800">{{ $addr->label }}</span>
                                                @if($addr->is_default)
                                                    <span class="text-[10px] font-bold text-terra-600 bg-terra-50 px-2 py-0.5 rounded-full border border-terra-100">Utama</span>
                                                @endif
                                            </span>
                                            <span class="text-sm font-semibold text-slate-900 mt-1">{{ $addr->recipient_name }} ({{ $addr->phone }})</span>
                                            <span class="text-xs text-slate-500 mt-1 leading-relaxed">{{ $addr->formatted_address }}</span>
                                        </span>
                                ...
                                <label class="relative flex p-4 border rounded-xl cursor-pointer focus:outline-none transition-all duration-200 {{ is_null($selectedAddressId) ? 'border-terra-500 bg-terra-50/30' : 'border-gray-200 hover:bg-slate-50' }}">
                            ...
                            @if($selectedAddressId)
                                ...
                                <div class="mt-6 pt-6 border-t border-gray-100 text-left">
                                    ...
                                    <input type="text" wire:model="notes" @disabled($isProcessing) placeholder="Contoh: Titip di pos satpam" class="w-full px-4 py-3 border border-slate-200 rounded-xl shadow-sm focus:border-terra-500 focus:ring-4 focus:ring-terra-500/10 focus:outline-none transition-all duration-200 disabled:bg-slate-50 disabled:text-slate-500">
                                </div>
                            @endif

                            <div class="mt-4 text-right">
                                <a href="{{ route('member.addresses') }}" class="text-xs font-bold text-terra-500 hover:text-terra-600 flex items-center justify-end gap-1">
                                    Kelola Buku Alamat
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 sm:p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" wire:model="name" @disabled($isProcessing) class="w-full px-4 py-3 border border-slate-200 rounded-xl shadow-sm focus:border-terra-500 focus:ring-4 focus:ring-terra-500/10 focus:outline-none transition-all duration-200 disabled:bg-slate-50 disabled:text-slate-500">
                            @error('name') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="font-display block text-sm font-bold text-slate-700 mb-2">Email</label>
                            <input type="email" wire:model="email" @disabled($isProcessing) class="w-full px-4 py-3 border border-slate-200 rounded-xl shadow-sm focus:border-terra-500 focus:ring-4 focus:ring-terra-500/10 focus:outline-none transition-all duration-200 disabled:bg-slate-50 disabled:text-slate-500">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="tel" wire:model="phone" @disabled($isProcessing) class="w-full px-4 py-3 border border-slate-200 rounded-xl shadow-sm focus:border-terra-500 focus:ring-4 focus:ring-terra-500/10 focus:outline-none transition-all duration-200 disabled:bg-slate-50 disabled:text-slate-500">
                            <p class="text-slate-400 text-[11px] mt-1.5 flex items-start gap-1 leading-normal">
                                <svg class="w-3.5 h-3.5 text-slate-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 sm:p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <textarea wire:model="address" @disabled($isProcessing) rows="3" class="w-full px-4 py-3 border border-slate-200 rounded-xl shadow-sm focus:border-terra-500 focus:ring-4 focus:ring-terra-500/10 focus:outline-none transition-all duration-200 disabled:bg-slate-50 disabled:text-slate-500"></textarea>
                            <p class="text-slate-400 text-[11px] mt-1.5 flex items-start gap-1 leading-normal">
                                <svg class="w-3.5 h-3.5 text-slate-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        ...
                        <div>
                            ...
                            <div wire:ignore x-data="{
                                ...
                                <select x-ref="selectProvince" @disabled($isProcessing) class="w-full border-gray-300 rounded-lg shadow-sm focus:border-terra-500 focus:ring focus:ring-terra-200 transition-shadow disabled:bg-gray-50">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="postal_code" @disabled($isProcessing) class="w-full px-4 py-3 border border-slate-200 rounded-xl shadow-sm focus:border-terra-500 focus:ring-4 focus:ring-terra-500/10 focus:outline-none transition-all duration-200 disabled:bg-slate-50 disabled:text-slate-500">
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 sticky top-24">
                        ...
                        <li class="py-3 flex justify-between">
                            <div class="flex-1 pr-4">
                                <h4 class="font-display text-sm font-bold text-slate-800 line-clamp-1">
                                    ...
                                    @if($item->variant)
                                        <span class="text-slate-500 font-normal">({{ $item->variant->name }})</span>
                                    @endif
                                </h4>
                                <div class="text-xs text-slate-500">{{ $item->quantity }} x Rp{{ number_format($item->variant ? $item->variant->final_price : ($item->product?->price ?? 0), 0, ',', '.') }}</div>
                            </div>
                            <span class="text-sm font-medium text-slate-900 whitespace-nowrap">Rp{{ number_format($item->subtotal, 0, ',', '.') }}</span>
                        </li>
                        @endforeach
                    </ul>
                    ...
                    <div class="space-y-3 mb-6 bg-gray-50 p-4 rounded-lg">
                        ...
                        <div class="border-t border-gray-200 pt-3 flex justify-between items-center mt-3">
                            ...
                            <span class="font-display font-black text-terra-600 text-fluid-h3">Rp{{ number_format($grandTotal, 0, ',', '.') }}</span>
                        </div>
                    </div>
