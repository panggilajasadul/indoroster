    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <nav class="flex text-xs font-semibold text-slate-400 dark:text-slate-500 gap-1.5 mb-2 uppercase tracking-wider">
                ...
                <span>/</span>
                <span class="text-slate-600 dark:text-slate-300">Pesanan Saya</span>
            </nav>
            <h1 class="font-display text-fluid-h1 font-black text-slate-900 dark:text-white tracking-tight">Pesanan Saya</h1>
            <p class="text-slate-500 dark:text-slate-400 mt-1">Pantau status produksi, pengantaran armada pabrik, dan riwayat belanja Anda.</p>
        </div>
        ...
        <div class="space-y-6">
            @if(count($orders) === 0)
                <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-soft-xs p-16 text-center">
                    ...
                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-2 max-w-sm mx-auto">Tidak ditemukan transaksi untuk kategori ini. Yuk, jelajahi katalog produk premium kami.</p>
                            ...
                            @foreach($order->items as $item)
                                <div class="py-4 first:pt-0 last:pb-0 flex gap-4">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        @endif
                                        <p class="text-xs text-slate-400 dark:text-slate-500 mt-1">Jumlah: {{ $item->quantity }} pcs</p>
                                    </div>
                        ...
                        <div class="bg-slate-50 dark:bg-slate-900/90 border-t border-slate-100 dark:border-slate-800 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                @else
                                    <div class="text-xs text-slate-500 dark:text-slate-400 max-w-sm leading-relaxed">
                                        Status: <strong class="text-slate-700 dark:text-slate-200">{{ $order->status_label }}</strong>. 
                            ...
                            <div class="flex flex-col items-end gap-2.5 shrink-0">
                                ...
                                <div class="flex gap-2.5 flex-wrap justify-end">
                                    ...
                                    @if($order->invoice)
                                        <a href="{{ URL::signedRoute('print.invoice', ['invoice' => $order->invoice->id]) }}" target="_blank" class="group inline-flex items-center justify-center border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 hover:border-slate-300 dark:hover:border-slate-600 text-slate-700 dark:text-white hover:text-slate-900 dark:hover:text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow-2xs transition-all duration-200 gap-1.5 cursor-pointer">
