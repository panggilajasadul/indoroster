<div class="bg-slate-50 min-h-screen py-6" wire:init="processPaymentStatus" @if($order->payment_status !== 'paid') wire:poll.8s="checkDatabaseStatus" @endif>
    <!-- Midtrans Snap JS -->
            ...
            <div class="p-4 sm:p-8 text-center">
                ...
                <div class="bg-slate-50 rounded-xl p-6 mb-6 text-left max-w-lg mx-auto border border-slate-100">
                    ...
                    <div class="flex justify-between mb-3 text-sm">
                        ...
                        @else
                            <span class="bg-yellow-100 text-yellow-700 px-2.5 py-0.5 rounded-full text-xs font-bold uppercase">{{ $order->payment_status }}</span>
                        @endif
                    </div>
                            ...
                            <div class="flex-1">
                                <p class="font-display font-bold text-slate-800">
                                    ...
                                    @if($item->product_variant_name)
                                        <span class="text-slate-500 font-normal text-xs">({{ $item->product_variant_name }})</span>
                                    @endif
                                </p>
                                <p class="text-slate-400 text-xs">{{ $item->quantity }}x @ Rp{{ number_format($item->product_price, 0, ',', '.') }}</p>
                            </div>
                            <span class="font-bold text-slate-700 ml-4">Rp{{ number_format($item->subtotal, 0, ',', '.') }}</span>
                        </div>
                        @endforeach
                    </div>
                ...
                <div class="flex flex-col gap-3 max-w-lg mx-auto">
                    @if($order->payment_status !== 'paid')
                        @if($order->snap_token)
                            <button onclick="continuePayment('{{ $order->snap_token }}')" class="font-display w-full bg-terra-600 hover:bg-terra-700 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-terra-600/20 text-center flex justify-center items-center gap-2">
