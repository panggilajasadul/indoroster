<div class="relative inline-block text-left" 
    ...
    <div x-show="open" 
        ...
        <div class="max-h-[70vh] overflow-y-auto">
            @forelse($notifications as $notification)
                ...
                <div wire:click="markAsRead('{{ $notification->id }}', '{{ $notification->data['url'] ?? '#' }}')" 
                     class="block px-5 py-4 hover:bg-slate-50 cursor-pointer transition duration-150 ease-in-out border-l-4 {{ $isUnread ? 'bg-blue-50/30 border-terra-500' : 'border-transparent' }}">
                    <div class="flex justify-between items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold text-gray-900 {{ $isUnread ? 'text-black' : 'text-gray-700' }}">
        ...
        <div class="px-4 py-3 bg-gray-50/50 border-t border-gray-100 text-center">
            <a href="{{ route('member.notifications') }}" wire:navigate class="text-sm font-medium text-terra-600 hover:text-terra-800 transition-colors">Lihat Semua Notifikasi</a>
        </div>
        @endif
    </div>
</div>
