...
<div class="min-h-screen bg-slate-50 dark:bg-slate-950 py-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
        <div class="relative rounded-3xl overflow-hidden bg-gradient-to-br from-white via-slate-50 to-blue-50/20 dark:from-slate-900 dark:via-slate-850 dark:to-slate-900 border border-slate-200/80 dark:border-slate-800 text-slate-900 dark:text-white p-8 sm:p-12 lg:p-16 shadow-soft-xl dark:shadow-2xl">
            <div class="absolute -right-20 -bottom-20 w-96 h-96 bg-blue-500/10 rounded-full blur-3xl pointer-events-none"></div>
            <div class="relative z-10 max-w-3xl">
                <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/30 text-blue-600 dark:text-blue-400 text-xs font-bold uppercase tracking-wider mb-6 shadow-xs">
                    <span>🏘️</span> Khusus Developer Perumahan & Kawasan
                </div>
                <h1 class="text-3xl sm:text-5xl font-black tracking-tight leading-tight text-slate-900 dark:text-white mb-6">
                    {!! !empty($page?->title) && $page->title !== 'Pengadaan Roster Skala Klaster (Khusus Developer Perumahan)' ? e($page->title) : 'Pengadaan Roster Beton untuk <span class="text-terra-500">Developer & Cluster</span>' !!}
                </h1>
                <p class="text-base sm:text-lg text-slate-600 dark:text-slate-300 mb-8 leading-relaxed">
                    {{ $page?->meta_description ?: 'Jaminan keseragaman warna dan mutu cetak padat untuk puluhan hingga ratusan unit rumah cluster, fasad gerbang utama (main gate), serta clubhouse dengan skema kontrak suplai terkunci.' }}
                </p>
                <div class="flex flex-wrap items-center gap-4">
                    <a href="{{ $waUrl }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center gap-2.5 px-7 py-3.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm sm:text-base shadow-lg shadow-emerald-600/25 transition-all hover:scale-[1.02]">
                        ...
                        Konsultasi Pengadaan Developer (WhatsApp)
                    </a>
                    <a href="{{ route('location.index') }}" class="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-white hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 font-bold text-sm transition-all border border-slate-300 dark:border-slate-700 shadow-xs">
                        Lihat Direktori Wilayah &rarr;
                    </a>
                </div>
            </div>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-20">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2">Keseragaman Mutu Skala Klaster</h3>
                <p class="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
                    Setiap keping diproduksi dengan formula komposisi dan cetakan presisi yang sama, menjamin keseragaman visual fasad dari unit pertama hingga unit terakhir.
                </p>
            </div>
            <div class="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <div class="text-3xl mb-4">📈</div>
                <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2">Kontrak Suplai Harga Terkunci</h3>
                <p class="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
                    Dapatkan jaminan stabilitas harga material roster selama periode pembangunan proyek klaster perumahan berlangsung, melindungi RAB Anda dari fluktuasi harga.
                </p>
            </div>
            <div class="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <div class="text-3xl mb-4">🛡️</div>
                <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2">Garansi 100% Ganti Baru</h3>
                <p class="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
                    Bebas risiko kerusakan pengiriman. Setiap keping cacat saat serah terima di gerbang proyek perumahan langsung diganti baru tanpa biaya tambahan.
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-20">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 pb-4 border-b border-slate-200 dark:border-slate-800">
            <div>
                <h2 class="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">
                    Katalog Roster Favorit Fasad Perumahan & Cluster
                </h2>
                <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
                    Jaminan keseragaman warna dan mutu cetak padat untuk puluhan hingga ratusan unit rumah klaster.
                </p>
            </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            ...
            @php
                ...
                $itemWaUrl = "https://wa.me/{$waNumber}?text=" . urlencode("Halo Tim IndoRoster, saya Developer ingin berkonsultasi pengadaan skala klaster perumahan untuk motif: {$product->name}.");
            @endphp
