<div class="min-h-[80vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-slate-50 dark:bg-slate-950">
    <div class="max-w-md w-full space-y-8 bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xl dark:shadow-2xl">
        ...
        <div class="text-center">
            ...
            <h2 class="font-display text-fluid-h2 font-black text-slate-900 dark:text-white tracking-tight">Selamat Datang</h2>
            <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Masuk ke akun Anda untuk pengalaman belanja yang lebih mudah.</p>
        </div>

        <!-- Success Alert -->
        @if (session()->has('success'))
            <div class="p-4 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-100 dark:border-emerald-900/40 text-emerald-700 dark:text-emerald-300 text-sm rounded-xl">
                {{ session('success') }}
            </div>
        @endif
        ...
        <form wire:submit.prevent="login" class="mt-8 space-y-6">
            <div class="space-y-5">
                ...
                <div x-data="{ show: false }">
                    ...
                    <div class="relative">
                        ...
                        <input id="password" :type="show ? 'text' : 'password'" wire:model="password" placeholder="••••••••" class="w-full pl-11 pr-11 py-3 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-xl shadow-sm focus:border-terra-500 focus:ring-4 focus:ring-terra-500/10 focus:outline-none transition-all duration-200 text-slate-800 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500">
            ...
            <div class="flex items-center">
                ...
                <label for="remember" class="ml-2 block text-sm font-medium text-slate-600 dark:text-slate-400">Ingat saya di perangkat ini</label>
            </div>

            <!-- Submit Button -->
            <div>
                <button type="submit" class="font-display w-full flex justify-center py-3.5 px-4 border border-transparent rounded-xl shadow-lg shadow-terra-500/20 text-sm font-bold text-white bg-terra-500 hover:bg-terra-600 focus:outline-none focus:ring-4 focus:ring-terra-500/20 transition-all duration-200 cursor-pointer">
                    Masuk Sekarang
                </button>
            </div>
        </form>
        ...
        <div class="text-center mt-6 pt-6 border-t border-slate-100 dark:border-slate-800">
            <p class="text-sm text-slate-500 dark:text-slate-400">
                Belum memiliki akun? 
                <a href="{{ route('register') }}" class="font-bold text-terra-500 hover:text-terra-600 dark:text-terra-400 dark:hover:text-terra-300 transition-colors">Daftar Akun Baru</a>
            </p>
        </div>
    </div>
</div>
