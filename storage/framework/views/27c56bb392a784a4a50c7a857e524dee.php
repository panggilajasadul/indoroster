<div class="py-12 bg-gray-50 min-h-screen" x-data="{ 
    ...
    activePhotoId: <?php if ((object) ('activePhotoId') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('activePhotoId'->value()); ?>')<?php echo e('activePhotoId'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('activePhotoId'); ?>')<?php endif; ?>, 
    photos: <?php if ((object) ('photos') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('photos'->value()); ?>')<?php echo e('photos'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('photos'); ?>')<?php endif; ?>,
    showToast: false,
    toastMessage: ''
}" x-init="
...
">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" wire:loading.class="opacity-50 transition-opacity">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-12 pb-6 border-b border-slate-200">
            ...
            <div class="flex flex-wrap gap-2.5 md:gap-3">
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $availableCategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <button 
                        type="button"
                        wire:click="setTab('<?php echo e($category); ?>')" 
                        class="group relative px-6 py-2.5 rounded-full text-[11px] font-bold uppercase tracking-[0.2em] transition-all duration-300 overflow-hidden
                        <?php echo e($activeTab === $category 
                            ? 'bg-terra-500 text-white shadow-[0_10px_20px_-5px_rgba(247,92,32,0.4)] translate-y-[-2px]' 
                            : 'bg-white text-slate-500 border border-slate-200 hover:border-terra-300 hover:text-terra-600 hover:shadow-md hover:translate-y-[-2px]'); ?>">
                        <span class="relative z-10"><?php echo e(strtoupper($category)); ?></span>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($activeTab !== $category): ?>
                            <div class="absolute inset-0 bg-terra-50 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    </button>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" id="gallery-container">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $photos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $image): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div 
                    @click="activeIndex = <?php echo e($index); ?>; activePhotoId = '<?php echo e($image['id']); ?>'; fullSizeModal = true"
                    wire:key="img-<?php echo e($activeTab); ?>-<?php echo e($loop->index); ?>"
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!empty($image['product'])): ?>
                        <div class="absolute top-4 left-4 z-10 pointer-events-auto">
                            <div 
                                @click.stop="window.location.href = '<?php echo e(url('/produk/' . $image['product']['slug'])); ?>'"
                                ...
                                <div class="flex flex-col leading-tight">
                                    <span class="truncate max-w-[110px] text-[9px] text-slate-500 group-hover/badge:text-white/80 font-medium transition-colors"><?php echo e($image['product']['name']); ?></span>
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-6 translate-y-4 group-hover:translate-y-0">
                        <span class="text-terra-400 text-xs font-bold uppercase tracking-widest mb-1"><?php echo e($image['category']); ?></span>
                        <h3 class="font-display text-white text-sm font-bold leading-tight drop-shadow-sm"><?php echo e($image['title']); ?></h3>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!empty($image['location'])): ?>
                        ...
                        <div class="flex items-center gap-3 mt-3 text-white/90 text-xs font-medium">
                            <span class="flex items-center gap-1.5 bg-black/30 backdrop-blur-sm px-2 py-0.5 rounded-full text-[10px]">
                                ...
                                <span x-text="photos[<?php echo e($index); ?>]?.likes_count || 0"></span>
                            </span>
                            <span class="flex items-center gap-1.5 bg-black/30 backdrop-blur-sm px-2 py-0.5 rounded-full text-[10px]">
                                ...
                                <span x-text="photos[<?php echo e($index); ?>]?.comments_count || 0"></span>
                            </span>
                        </div>
                    </div>
        ...
        <div class="w-full h-full flex flex-col md:flex-row relative">
            ...
            <div class="flex-1 h-full flex items-center justify-center relative bg-black/20 p-4">
                ...
                <div class="absolute right-4 bottom-24 z-[115] flex flex-col items-center gap-4 text-white md:hidden">
                    ...
                    <!-- Like Button -->
                    <button 
                        @click.stop="if (!isLoggedIn) { window.location.href = '<?php echo e(route('login')); ?>'; } else { $wire.toggleLike(photos[activeIndex].id) }" 
                    ...
                    <!-- Comment Button -->
                    <button 
                        @click.stop="if (!isLoggedIn) { window.location.href = '<?php echo e(route('login')); ?>'; } else { activePhotoId = photos[activeIndex].id; commentDrawerOpen = true; }" 
                        class="flex flex-col items-center justify-center group focus:outline-none cursor-pointer">
                        <div class="w-10 h-10 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center transition-all duration-200 border border-white/10 active:scale-95 text-white">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5">
...
<?php /**PATH C:\xampp\htdocs\indoroster\resources\views/livewire/gallery.blade.php ENDPATH**/ ?>