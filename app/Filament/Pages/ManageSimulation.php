...
class ManageSimulation extends Page implements HasForms, HasTable
{
    ...
    public function refreshStats(): void
    {
        ...
        $this->totalLikes = \App\Models\Like::count();
        $this->seededLikes = \App\Models\Like::whereHas('user', function($q) {
    ...
    public function videoForm(Form $form): Form
    {
        return $form
            ->schema([
                \Filament\Forms\Components\Select::make('video_id')
    ...
    public function photoForm(Form $form): Form
    {
        return $form
            ->schema([
                \Filament\Forms\Components\Select::make('photo_id')
        ...
        return $form
            ->schema([
                \Filament\Forms\Components\Select::make('media_type')
                    ...
                    ->required()
                    ->live()
                    ->afterStateUpdated(fn (\Filament\Forms\Set $set) => $set('gallery_id', null)),
                \Filament\Forms\Components\Select::make('gallery_id')
                    ...
                    ->searchable()
                    ->required()
                    ->disabled(fn (\Filament\Forms\Get $get) => ! $get('media_type')),
                \Filament\Forms\Components\TextInput::make('quantity')
                    ->label('Jumlah Like')
                    ->numeric()
                    ->default(100)
